	...
	if(transitionController.isStateNew()){
        ...
        switch (transitionController.getCurrentState()) {
            ...
            case TRANSITION_CLUSTERMAP_IN:
                ...
                if(run.actCount > 0){
                    vector<string> newTopics = storyEngine.getValidTopicsForNextAct(run);
                    vector<CloudsVisualSystemPreset> potentialPresets = visualSystems.getPresetsForKeywords(newTopics,"",true);
                    cout<<"Potentnial presets size: "<<potentialPresets.size()  <<endl;
                    if(potentialPresets.size()== 0){
                        ofLogError("CloudsPlaybackController::updateTransition")<<"No valid topic for interlude, running cluster map"<<endl;
                        clusterMap->traverse();
                        ...
                        cout<<"Playing preset : "<<interlude.presetName<<" for system "<<interlude.systemName<<endl;;
                        currentVisualSystemPreset = interlude;
                        if(currentVisualSystemPreset.system == NULL){
                            cout<<"System is null"<<endl;
                        }
                        currentVisualSystemPreset.system->setDrawToScreen( false );

                        currentVisualSystemPreset.system->loadPresetGUISFromName( interlude.presetName );
