...
namespace GPUParticles {
	...
	void Controller::setParticleCount(int particles){
		numParticles = particles;
	}
	
	void Controller::setup(){
		cout << "INIT STATE " << shaderDirectory << endl;
		state.init(numParticles, shaderDirectory);
	...
	void Controller::reloadShaders(){
		...
		null.load(shaderDirectory + "default.vert", shaderDirectory + "null.frag");
		forceSet.load(shaderDirectory);
		state.loadShader();
