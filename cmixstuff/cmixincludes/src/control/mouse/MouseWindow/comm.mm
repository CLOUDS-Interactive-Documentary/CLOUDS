...
#include "comm.h"
#include "mouse_ipc.h"
#include "MouseView-CInterface.h"
...
void sendCoordinates(const double xscaled, const double yscaled)
{
...
void *listenerLoop(void *context)
{
	...
	while (_runThread) {
				...
				if (mouseView) {
					switch (packet->type) {
						case kPacketConfigureXLabelPrefix:
							MouseViewConfigureXLabelPrefix(mouseView, packet->id, packet->data.str);
							break;
						case kPacketConfigureYLabelPrefix:
							MouseViewConfigureYLabelPrefix(mouseView, packet->id, packet->data.str);
							break;
						case kPacketConfigureXLabelUnits:
							MouseViewConfigureXLabelUnits(mouseView, packet->id, packet->data.str);
							break;
						case kPacketConfigureYLabelUnits:
							MouseViewConfigureYLabelUnits(mouseView, packet->id, packet->data.str);
							break;
						case kPacketConfigureXLabelPrecision:
							MouseViewConfigureXLabelPrecision(mouseView, packet->id, packet->data.ival);
							break;
						case kPacketConfigureYLabelPrecision:
							MouseViewConfigureYLabelPrecision(mouseView, packet->id, packet->data.ival);
							break;
						case kPacketUpdateXLabel:
							MouseViewUpdateXLabelValue(mouseView, packet->id, packet->data.dval);
							labelsUpdated = true;
							break;
						case kPacketUpdateYLabel:
							MouseViewUpdateYLabelValue(mouseView, packet->id, packet->data.dval);
							labelsUpdated = true;
							break;
		...
		if (labelsUpdated && mouseView)	// draw only the final state for this polling
			MouseViewDoUpdate(mouseView);
