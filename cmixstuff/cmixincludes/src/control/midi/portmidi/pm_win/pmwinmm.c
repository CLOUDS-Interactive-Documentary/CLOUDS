...
static void FAR PASCAL winmm_in_callback(
     HMIDIIN hMidiIn,   /* midiInput device RtcHandle */
     WORD wMsg,         /* midi msg */
...
static PmError winmm_out_open(PmInternal *midi, void *driverInfo)
{
    ...
    if (midi->latency == 0) { 
        ...
        pm_hosterror = midiOutOpen((LPHMIDIOUT) &m->handle.out, /* device RtcHandle */
                           dwDevice, /* device ID  */
        ...
        pm_hosterror = midiStreamOpen(&m->handle.stream, /* device RtcHandle */
