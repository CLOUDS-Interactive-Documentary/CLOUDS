...
//	float currentTraverseReveal = map(freshTraversalPosition, percentTraverseRevealed, 1.0, 0.0, 1.0);
	...
	float segmentVertexPosition = freshTraversalPosition * segementVertCount;
	float colorMix = map(segementVertCount*traverseReveal - segmentVertexPosition, 0.0, colorFalloff*10., 0., 1.0);
...
//	gl_FragColor.rgb = vec3( ( (trailVertCount-(segementVertCount-currentVert)) - totalTraversalPosition) / segementVertCount );
}
